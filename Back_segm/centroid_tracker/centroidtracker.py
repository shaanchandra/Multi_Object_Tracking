...
class CentroidTracker:
	...
	def update(self, rects):
		...
		# otherwise, are are currently tracking objects so we need to try to match the input centroids to existing object centroids
			...
			# compute the distance between each pair of object centroids and input centroids, respectively -- our goal will be to match an input centroid to an existing object centroid
			...
			if D.shape[0] >= D.shape[1]:
				...
				for row in unusedRows:
					...
					# for warrants deregistering the object if self.disappeared[objectID] > self.maxDisappeared:
					self.deregister(objectID)
